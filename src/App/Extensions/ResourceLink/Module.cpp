...
namespace
{
constexpr auto ModuleName = "ResourceLink";
}

std::string_view App::ResourceLinkModule::GetName()
{
    return ModuleName;
}

bool App::ResourceLinkModule::Load()
{
...
bool App::ResourceLinkModule::Unload()
{
...
void App::ResourceLinkModule::Configure()
{
    s_links.clear();
        ...
        for (const auto& [targetPath, linkPaths] : config.links)
        {
            for (const auto& linkPath : linkPaths)
            {
                if (linkPath == targetPath)
                {
                    ...
                    {
                        LogError("|{}| Link \"{}\" points to itself.", ModuleName, config.paths[linkPath]);
                ...
                if (depot->ResourceExists(linkPath))
                {
                    if (!invalidPaths.contains(linkPath))
                    {
                        LogError("|{}| Link \"{}\" is an existing resource.", ModuleName, config.paths[linkPath]);
                        invalidPaths.insert(linkPath);
                    }
                    continue;
                }
    ...
    for (const auto& linkPath : cyclicLinks)
    {
        if (!invalidPaths.contains(linkPath))
        {
            LogError("|{}| Link \"{}\" is cyclic.", ModuleName, s_paths[linkPath]);
            invalidPaths.insert(linkPath);
        }
        ...
        if (!depot->ResourceExists(targetPath))
        {
            if (!invalidPaths.contains(targetPath))
            {
                LogError(R"(|{}| Link "{}" points to a non-existent resource "{}".)",
                         ModuleName, s_paths[linkPath], s_paths[targetPath]);
...
void App::ResourceLinkModule::OnResourceRequest(Red::ResourceLoader*, Red::SharedPtr<Red::ResourceToken<>>&,
...
bool App::ResourceLinkModule::OnResourceCheck(Red::ResourceDepot* aDepot, Red::ResourcePath aPath)
{
